...
(defn make-servlet-context-handler
  ([handler] (make-servlet-context-handler handler Context/SESSIONS))
  ([handler options]
...
(defn ^Server run-jetty
  ...
  Options:
    ...
    :servlet-context-maker  function to generate jetty context handler
    :ssl?           - Use SSL.
  ...
  (let [^Server s (create-server (dissoc options :configurator))]
    ...
    (doto s
      (.setHandler
       ((:servlet-context-maker options make-servlet-context-handler)
        handler))
      (.start))
